...
# таймаут функции и апи-гетевай 5 минут
...
class UserClient(YDBClient):
    ...
    async def insert_user(self, user: User) -> User:
        ...
        await self.execute_query(
            ...
            DECLARE $free_generate AS Uint8;
    ...
    async def update_field_free_generate(self, telegram_id: int, free_generate: int):
        ...
        params = {"$telegram_id": (telegram_id, ydb.PrimitiveType.Uint64),
                  "$free_generate": (free_generate, ydb.PrimitiveType.Uint8)}
    ...
    def _to_params(self, user: User) -> dict:
        return {
            ...
            "$free_generate": (user.free_generate, ydb.PrimitiveType.Uint8),
