...
async def cmd_start(message: types.Message):
    ...
    await message.answer_photo(texts["IMAGES"]["start"])
...
async def handle_photo_or_document(message: types.Message):
    ...
    if user.free_generate > 0:
        ...
        if photo_file is None:
            ...
            async with UserClient() as user_client:
                await user_client.update_field_free_generate(user_id, user.free_generate - 1)
...
async def on_successful_payment(message: types.Message):
    ...
    try:
        ...
        logger.error("Ошибка удаления сообщений:", e)
    ...
    try:
        ...
        logger.error("Ошибка при обработке изображения Nano Banano:", e)
        photo_file = None
